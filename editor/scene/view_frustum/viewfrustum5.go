...
func (view *ViewFrustum5) TriangleInsidePlane(plane geom.Plane, triangle geom.Triangle) ([]geom.Triangle, bool) {
	inside := make([]homocoord.Vec3, 0)
	outside := make([]homocoord.Vec3, 0)
	for i := 0; i < 2; i++ {
		vertex := triangle.GetVertex(i)
		if plane.DistanceFromPlane(vertex) > 0 {
	...
	case 1:
		iPoint := inside[0]
		edgePoint1, _ := plane.IntersectsLine(iPoint, outside[0])
		edgePoint2, _ := plane.IntersectsLine(iPoint, outside[1])

		return []geom.Triangle{
			*geom.NewTriangle(iPoint, edgePoint1, edgePoint2, triangle.GetColor()),
		}, false
	case 2:
		iPoint1 := inside[0]
		iPoint2 := inside[1]
		...
		x, _ := plane.IntersectsLine(iPoint1, outside)
		y, _ := plane.IntersectsLine(iPoint2, outside)
		return []geom.Triangle{
			*geom.NewTriangle(iPoint1, x, iPoint2, triangle.GetColor()),
			*geom.NewTriangle(x, y, iPoint2, triangle.GetColor()),
		}, false
	default:
